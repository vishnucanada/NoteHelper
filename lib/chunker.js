...
(function () {
    ...
    // ---- PDF extraction ----
    // onProgress(stage, current, total) is optional; called as ('extracting', p, numPages).
    async function extractPages(file, onProgress) {
        ...
        for (let p = 1; p <= pdf.numPages; p++) {
            ...
            if (onProgress) onProgress('extracting', p, pdf.numPages);
        }
        return pages;
    }

    // Returns { chunks, fullText }. Each chunk: {doc_id, filename, page, chunk_idx, text}
    // onProgress is forwarded to extractPages (optional).
    async function chunkPdf(file, docId, filename, onProgress) {
