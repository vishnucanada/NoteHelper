...
(function () {
    ...
    function db() {
        ...
        _dbPromise = new Promise((resolve, reject) => {
            ...
            // Don't cache a transient open failure forever — clear it so the next
            // call retries instead of poisoning the whole session until reload.
            _dbPromise = null;
            throw err;
        });
        return _dbPromise;
    ...
    // ---- documents ----
    // onProgress(stage, current, total) is optional; called as ('embedding', done, total).
    async function addDocument(docId, filename, summary, chunks, onProgress) {
        if (!chunks.length) return;
        // embed all chunk texts in batches
        const embeddings = [];
        for (let i = 0; i < chunks.length; i += EMBED_BATCH) {
            const batch = chunks.slice(i, i + EMBED_BATCH).map((c) => c.text);
            embeddings.push(...(await NH.gemini.embed(batch, 'RETRIEVAL_DOCUMENT')));
            if (onProgress) onProgress('embedding', Math.min(i + batch.length, chunks.length), chunks.length);
        }
        const d = await db();
    ...
    async function retrieve(query, docIds, k = 4) {
        const [qVec] = await NH.gemini.embed([query], 'RETRIEVAL_QUERY');
        return retrieveByVector(qVec, docIds, k);
    }

    // Same as retrieve() but takes a precomputed query vector, so a fan-out can
    // embed each unique query once and reuse it across routed documents.
    async function retrieveByVector(qVec, docIds, k = 4) {
        let candidates;
    ...
    NH.store = {
        addDocument, listDocuments, getDocument, deleteDocument, retrieve, retrieveByVector,
