...
(function () {
    ...
    async function retrievePhase(state, onEvent) {
        ...
        const results = await Promise.all(
            jobs.map(({ q, d }) => NH.store.retrieve(q, d ? [d] : null, k))
        );
    ...
    function extractClaims(answer) {
        ...
        for (const sentence of answer.trim().split(SENTENCE_SPLIT_RE)) {
            ...
            if (nums.length) {
                claims.push({ claim: sentence.replace(CITATION_RE, '').trim(), chunk_nums: nums });
            }
        }
        return claims;
    }
    ...
    async function verifyClaim(claim, chunkText) {
        ...
        try {
            ...
            // lenient: a transient verifier failure shouldn't fail the answer (graph.py:304)
            return { supported: true, reason: `(verifier error, defaulting to pass: ${e.message})` };
        }
    }
    ...
    async function criticNode(state) {
        ...
        for (const entry of extracted) {
            for (const n of entry.chunk_nums) {
                ...
                const verdict = await verifyClaim(entry.claim, chunk.text);
                citations.push({
                    n,
                    claim: entry.claim,
                    chunk_id: chunk.id,
